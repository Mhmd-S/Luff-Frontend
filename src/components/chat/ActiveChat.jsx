      ...
      <div className='relative w-full h-full grid grid-rows-[10%_77%_13%] grid-cols-1  md:shadow-lg md:rounded-md md:w-2/5'>
        ...
        <div className='w-full h-full flex bg-white items-center py-6 border-b-2 md:rounded-t-md'>
          ...
          <button className='w-fit h-full p-3  flex flex-row items-center' onClick={ ()=> { setRecipient(null); setChatId(null) } }>
            <FontAwesomeIcon icon={faChevronLeft} className='text-2xl text-[#a168ff]'/>
          </button>

          <div className='flex items-center'>
            <img src={recipient.profilePictures[0]} alt='profileImage' className='w-10 h-10 ml-4 rounded-full'/>
            <p className='p-4 text-lg font-semibold text-[#023c64]'>{recipient.name}</p>
          </div>
          
        </div>

        <ul className='w-full h-full bg-[#fafafc] p-2 text-sm overflow-y-scroll scrollbar:bg-blue-500 scrollbar scrollbar-thumb-blue-500 scrollbar-track-gray-200' ref={chatWindowRef}>
          <li ref={topRef}></li>
